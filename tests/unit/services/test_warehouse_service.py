...
class TestWarehouseService:
    ...
    @pytest.mark.asyncio
    async def test_get_warehouse_success(self, warehouse_service, sample_warehouse):
        """Test get_warehouse successful retrieval"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=sample_warehouse)
        
        result = await warehouse_service.get_warehouse(1)
        
        assert result == sample_warehouse
        warehouse_service.warehouse_repo.get.assert_called_once_with(1)

    @pytest.mark.asyncio
    async def test_get_warehouse_not_found(self, warehouse_service):
        """Test get_warehouse when warehouse not found"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=None)
        
        with pytest.raises(WarehouseNotFoundError, match="Warehouse 1 not found"):
            await warehouse_service.get_warehouse(1)
    ...
    @pytest.mark.asyncio
    async def test_add_product_to_warehouse_success(self, warehouse_service, sample_warehouse, sample_product):
        ...
        await warehouse_service.add_product_to_warehouse(warehouse_id=1, product_id=1, quantity=10)
    ...
    @pytest.mark.asyncio
    async def test_add_product_to_warehouse_negative_quantity(self, warehouse_service):
        ...
        with pytest.raises(InvalidQuantityError, match="Quantity must be positive"):
            await warehouse_service.add_product_to_warehouse(warehouse_id=1, product_id=1, quantity=-5)

    @pytest.mark.asyncio
    async def test_add_product_to_warehouse_zero_quantity(self, warehouse_service):
        """Test add_product_to_warehouse with zero quantity"""
        with pytest.raises(InvalidQuantityError, match="Quantity must be positive"):
            await warehouse_service.add_product_to_warehouse(warehouse_id=1, product_id=1, quantity=0)

    @pytest.mark.asyncio
    async def test_add_product_to_warehouse_warehouse_not_found(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_add_product_to_warehouse_product_not_found(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_add_product_to_warehouse_large_quantity(self, warehouse_service, sample_warehouse, sample_product):
        ...
        await warehouse_service.add_product_to_warehouse(warehouse_id=1, product_id=1, quantity=1000000)
    ...
    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_success(self, warehouse_service, sample_warehouse, sample_product):
        """Test remove_product_from_warehouse successful removal"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=sample_warehouse)
        warehouse_service.product_repo.get = Mock(return_value=sample_product)
        warehouse_service._get_warehouse_product_quantity = AsyncMock(return_value=50)
        warehouse_service.warehouse_repo.remove_product_from_warehouse = Mock()
        
        await warehouse_service.remove_product_from_warehouse(warehouse_id=1, product_id=1, quantity=10)
    ...
    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_negative_quantity(self, warehouse_service):
        """Test remove_product_from_warehouse with negative quantity"""
        with pytest.raises(InvalidQuantityError, match="Quantity must be positive"):
            await warehouse_service.remove_product_from_warehouse(warehouse_id=1, product_id=1, quantity=-1)

    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_zero_quantity(self, warehouse_service):
        """Test remove_product_from_warehouse with zero quantity"""
        with pytest.raises(InvalidQuantityError, match="Quantity must be positive"):
            await warehouse_service.remove_product_from_warehouse(warehouse_id=1, product_id=1, quantity=0)

    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_warehouse_not_found(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_product_not_found(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_insufficient_stock(self, warehouse_service, sample_warehouse, sample_product):
        """Test remove_product_from_warehouse with insufficient stock"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=sample_warehouse)
        warehouse_service.product_repo.get = Mock(return_value=sample_product)
        warehouse_service._get_warehouse_product_quantity = AsyncMock(return_value=5)
        
        with pytest.raises(InsufficientStockError, match="Insufficient stock in warehouse: only 5 items available"):
            await warehouse_service.remove_product_from_warehouse(warehouse_id=1, product_id=1, quantity=10)

    @pytest.mark.asyncio
    async def test_remove_product_from_warehouse_exact_stock(self, warehouse_service, sample_warehouse, sample_product):
        """Test remove_product_from_warehouse with exact available stock"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=sample_warehouse)
        warehouse_service.product_repo.get = Mock(return_value=sample_product)
        warehouse_service._get_warehouse_product_quantity = AsyncMock(return_value=10)
        warehouse_service.warehouse_repo.remove_product_from_warehouse = Mock()
    ...
    @pytest.mark.asyncio
    async def test_get_warehouse_inventory_success(self, warehouse_service, sample_warehouse, sample_inventory_item):
        ...
        result = await warehouse_service.get_warehouse_inventory(warehouse_id=1)
    ...
    @pytest.mark.asyncio
    async def test_get_warehouse_inventory_warehouse_not_found(self, warehouse_service):
        """Test get_warehouse_inventory when warehouse not found"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=None)
        
        with pytest.raises(WarehouseNotFoundError, match="Warehouse 1 not found"):
            await warehouse_service.get_warehouse_inventory(warehouse_id=1)

    @pytest.mark.asyncio
    async def test_get_warehouse_inventory_empty(self, warehouse_service, sample_warehouse):
        ...
        result = await warehouse_service.get_warehouse_inventory(1)
    ...
    @pytest.mark.asyncio
    async def test_transfer_product_success(self, warehouse_service, sample_warehouse, sample_product):
        ...
        await warehouse_service.transfer_product(
            from_warehouse_id=1,
            to_warehouse_id=2,
            product_id=1,
            quantity=10
    ...
    @pytest.mark.asyncio
    async def test_transfer_product_negative_quantity(self, warehouse_service):
        ...
        with pytest.raises(InvalidQuantityError, match="Transfer quantity must be positive"):
            await warehouse_service.transfer_product(from_warehouse_id=1, to_warehouse_id=2, product_id=1, quantity=-5)

    @pytest.mark.asyncio
    async def test_transfer_product_zero_quantity(self, warehouse_service):
        """Test transfer_product with zero quantity"""
        with pytest.raises(InvalidQuantityError, match="Transfer quantity must be positive"):
            await warehouse_service.transfer_product(from_warehouse_id=1, to_warehouse_id=2, product_id=1, quantity=0)

    @pytest.mark.asyncio
    async def test_transfer_product_same_warehouse(self, warehouse_service):
        """Test transfer_product to same warehouse"""
        with pytest.raises(ValidationError, match="Cannot transfer to the same warehouse"):
            await warehouse_service.transfer_product(from_warehouse_id=1, to_warehouse_id=1, product_id=1, quantity=10)

    @pytest.mark.asyncio
    async def test_transfer_product_from_warehouse_not_found(self, warehouse_service):
        """Test transfer_product when source warehouse not found"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(side_effect=[None, None])
        
        with pytest.raises(WarehouseNotFoundError, match="Warehouse 1 not found"):
            await warehouse_service.transfer_product(from_warehouse_id=1, to_warehouse_id=2, product_id=1, quantity=10)

    @pytest.mark.asyncio
    async def test_transfer_product_to_warehouse_not_found(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_transfer_product_product_not_found(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_transfer_product_insufficient_stock(self, warehouse_service, sample_warehouse, sample_product):
    ...
    @pytest.mark.asyncio
    async def test_get_all_warehouses_with_inventory_summary_success(self, warehouse_service, sample_warehouse, sample_inventory_item):
        ...
        result = await warehouse_service.get_all_warehouses_with_inventory_summary()
    ...
    @pytest.mark.asyncio
    async def test_get_all_warehouses_with_inventory_summary_empty(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_get_all_warehouses_with_inventory_summary_empty_inventory(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_get_all_warehouses_with_inventory_summary_multiple_warehouses(self, warehouse_service, sample_inventory_item):
        ...
        warehouse_service.warehouse_repo.get_all = Mock(return_value=warehouses_dict)
        warehouse_service.warehouse_repo.get_warehouse_inventory = Mock(side_effect=[[sample_inventory_item], [sample_inventory_item]])
        ...
        assert result[1]["inventory_summary"]["total_items"] == 50
    ...
    @pytest.mark.asyncio
    async def test_get_all_warehouses_success(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_get_all_warehouses_empty(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_transfer_all_inventory_success(self, warehouse_service, sample_warehouse):
        ...
        result = await warehouse_service.transfer_all_inventory(from_warehouse_id=1, to_warehouse_id=2)
    ...
    @pytest.mark.asyncio
    async def test_transfer_all_inventory_same_warehouse(self, warehouse_service):
        """Test transfer_all_inventory to same warehouse"""
        with pytest.raises(ValidationError, match="Cannot transfer to the same warehouse"):
            await warehouse_service.transfer_all_inventory(from_warehouse_id=1, to_warehouse_id=1)

    @pytest.mark.asyncio
    async def test_transfer_all_inventory_from_warehouse_not_found(self, warehouse_service):
        """Test transfer_all_inventory when source warehouse not found"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=None)
        
        with pytest.raises(WarehouseNotFoundError, match="Warehouse 1 not found"):
            await warehouse_service.transfer_all_inventory(from_warehouse_id=1, to_warehouse_id=2)

    @pytest.mark.asyncio
    async def test_transfer_all_inventory_to_warehouse_not_found(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_transfer_all_inventory_empty_inventory(self, warehouse_service, sample_warehouse):
    ...
    @pytest.mark.asyncio
    async def test_delete_warehouse_success(self, warehouse_service, sample_warehouse):
        ...
        await warehouse_service.delete_warehouse(1)
    ...
    @pytest.mark.asyncio
    async def test_delete_warehouse_not_found(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_get_warehouse_product_quantity_found(self, warehouse_service, sample_warehouse, sample_product):
        """Test _get_warehouse_product_quantity when product is found"""
        # Mock dependencies
        inventory_item = InventoryItem(product_id=1, quantity=50)
        warehouse_service.warehouse_repo.get_warehouse_inventory = Mock(return_value=[inventory_item])
        
        result = await warehouse_service._get_warehouse_product_quantity(warehouse_id=1, product_id=1)
        
        assert result == 50

    @pytest.mark.asyncio
    async def test_get_warehouse_product_quantity_not_found(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_get_warehouse_product_quantity_empty_inventory(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_service_integration_add_remove_cycle(self, warehouse_service, sample_warehouse, sample_product):
        ...
        # Add product
        await warehouse_service.add_product_to_warehouse(warehouse_id=1, product_id=1, quantity=25)
        
        # Remove product
        await warehouse_service.remove_product_from_warehouse(warehouse_id=1, product_id=1, quantity=15)
    ...
    @pytest.mark.asyncio
    async def test_service_integration_transfer_and_summary(self, warehouse_service, sample_warehouse, sample_product):
        ...
        # Get summary
        summary = await warehouse_service.get_all_warehouses_with_inventory_summary()
    ...
    @pytest.mark.asyncio
    async def test_service_with_large_quantities(self, warehouse_service, sample_warehouse, sample_product):
        """Test service methods with large quantities"""
        # Mock dependencies
        warehouse_service.warehouse_repo.get = Mock(return_value=sample_warehouse)
        warehouse_service.product_repo.get = Mock(return_value=sample_product)
        warehouse_service._get_warehouse_product_quantity = AsyncMock(return_value=1000000)
        warehouse_service.warehouse_repo.add_product_to_warehouse = Mock()
        warehouse_service.warehouse_repo.remove_product_from_warehouse = Mock()
        ...
        await warehouse_service.remove_product_from_warehouse(warehouse_id=1, product_id=1, quantity=500000)

    @pytest.mark.asyncio
    async def test_service_with_unicode_data(self, warehouse_service):
    ...
    @pytest.mark.asyncio
    async def test_service_with_special_characters(self, warehouse_service):
